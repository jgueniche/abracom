...
export default async function AssessmentsPage({
  ...
  if (!editor && !canSeeAssessments(user.roles, cls.school_id))
    return (
      <Column>
        <EmptyState icon={GraduationCapIcon} title={t("noAccess")} />
      </Column>
    );
  ...
  if (!period)
    return (
      <Column>
        <EmptyState icon={GraduationCapIcon} title={t("noPeriods")} />
      </Column>
    );
  ...
  // A class whose level carries no skill catalogue used to render the matrix
  // anyway: a column of ten names, a legend for a scale with nothing to apply
  // it to, and a "save the grid" button under an empty grid. The seed only
  // fills PS, MS and GS, so six levels out of nine — the whole of élémentaire —
  // looked broken rather than unconfigured. Say what is missing instead.
  if (skills.length === 0)
    return (
      <Column>
        <div className="flex flex-col gap-6">
          ...
          <EmptyState
            ...
            description={t("noSkillsHint", {
              level:
                (locale === "en" ? cls.level?.label_en : cls.level?.label_fr) ??
                cls.level?.code ??
                cls.name,
            })}
          />
